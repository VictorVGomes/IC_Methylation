...
densityPlot(MSet, sampGroups = pData(MSet)$statusFumante)

# outros plots estão disponíveis, como do densityBeanPlot:
densityBeanPlot(MSet, sampGroups = pData(MSet)$statusFumante)
...
falharam <- detP > 0.05
plot(as.factor(pData(rgSet_combined)$geo_accession), colMeans(falharam), main='Taxa média de falha por amostra', xlab='Amostra', ylab='taxa de probes com p-valor de detecção > 0.05')

amostra_ruim = which(colMeans(falharam)>0.05)
