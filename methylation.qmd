...
quais_amostras_detp = names(which(amostra_ruim_det==TRUE))
```
...
whichBadNames = sampleNames(rgSet_combined[, whichBad])

amostrasARemover = union.Vector(quais_amostras_detp, whichBadNames)
...
plot(as.factor(pData(rgSet_combined)$geo_accession), colMeans(falharam),
     ...
     ylab='taxa de probes com p-valor de detecção > 0.01')
```
...
mdsPlot(getM(MSet_raw),
        sampNames = rgSet_combined$geo_accession,
        sampGroups=rgSet_combined$statusFumante, )
```
...
fviz_pca_ind(prcomp(rgset_samp, center = T, scale.=T), habillage = MSet_raw$statusFumante,
             axes = c(1, 3), addEllipses = T, ellipse.level=0.95)
```


```{r}
# removendo amostras e probes problemáticos
rgSet_combinedFiltrado = rgSet_combined[-probesRemover,
                  which(!amostrasARemover %in% sampleNames(rgSet_combined))]


rm(rgSet_combinedFiltrado)
```
...
MSetQuantile = preprocessQuantile(object = rgSet_combined,
                          fixOutliers = TRUE, 
